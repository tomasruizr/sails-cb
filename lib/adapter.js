  ...
  var adapter = {
    ...
    update: function (connection, collection, options, values, cb) {
      ...
      if (options && options.where && options.where.id){
        ...
        self.query(connection, collection, n1ql.buildUpdate(collection, conn.bucketName, options, values), options, function (err, res) {
          cb(err, n1ql.normalizeResponse(res, collection));
        });
      }
    },
    ...
    destroy: function (connection, collection, options, cb) {
      ...
      if (options.where.id) {
        ...
        self.query(connection, collection, n1ql.buildDelete(collection, conn.bucketName, options), options, function (err, res) {
          cb(err, n1ql.normalizeResponse(res, collection));
        });
      }
      ...
      //   } else {
      //     self.query(connection, collection, n1ql.buildDelete(collection, conn.bucketName, options), options, function (err, res) {
      //       next(err, connections.normalizeResponse(res, collection));
      //     });
      //   }  
      // }],function(err, doc) {
      //     cb(err, doc);
