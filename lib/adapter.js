...
module.exports = (function() {
	...
	var adapter = {
		 ...
		 */
		registerConnection: async function(connection, colls, cb) {
			...
			// Register Connection.
			var conn = await connections.add(connection, this.defaults);
			var strategy = conn.options.idStrategy || this.defaults.idStrategy;
			// Register Collections.
			async.forEachOf(
				colls,
				function(atts, name, next) {
					collections[name] = _.pick(atts, "primaryKey", "definition");
					// in case of increment id strategy ensure Counters.
					if (strategy === "increment") {
						return connections.ensureCounter(conn.bucket, name, next);
					}
					next();
				},
				cb
		...
		drop: function(connection, collection, relations, cb) {
			...
			if (relations && relations.length > 0) {
				throw new Error(
					"There are relations in the Drop method... ??? Never hit this in the tests, Proceed to handle this case as you need. TR"
				);
			}
			...
			this.query(connection, collection, query, {}, function(err, res) {
				console.log(
					"Found " + res.length + " results to delete in " + collection
				);
				cb(err, res);
			});
		},
		...
		/**
		 * Method to handle the Select Operation with relations between the waterline models.
		 *
		 * @method join
		 *
		 * @param  {String}   connectionName Connection Name
		...
		find: function(connection, collection, options, cb) {
			...
			if (
				options &&
				options.where &&
				options.where[collections[collection].primaryKey]
			) {
			...
			if (id && !Array.isArray(id) && _.keys(options.where).length === 1) {
				...
				if (id) {
					// we can assume it's an array of ids.
					if (_.isArray(id)) {
		...
		create: function(connection, collection, values, options, cb) {
			...
			// Ensure is a customPK
			var idPassedIn =
				!!values[collections[collection].primaryKey] && !opts.removePK;

			var id;
			// In case the idStrategy is not increment avoid the recursive call to generate a new ID.
			if (
				opts.idStrategy !== "increment" &&
				!values[collections[collection].primaryKey]
			) {
				id = connections.getNewId(conn.bucket, collection, options.idStrategy);
			}

			if (values[collections[collection].primaryKey] || id) {
				id = values[collections[collection].primaryKey] || id;
				if (typeof id !== "string") {
					id = id.toString();
				}
				// Ensure the collection name as a prefix to handle Document Types inside a Bucket.
				// id = id.startsWith(collection + '::') ? id : collection + '::' + id;
				id = utils.prepareIds(id, collection);
				if (opts.removePK) {
					// Remove the primary key for the document if the primary key was generated so it wont be stored in the document value (that's what the " Document key" is there for).
					delete values[collections[collection].primaryKey];
				}
				var doc;
				conn.bucket.insert(id, values, opts, function(err) {
					function insertResponse() {
						...
						// Set the id of the documents in case is not a custom PK
						doc[collections[collection].primaryKey] =
							values[collections[collection].primaryKey] || id;
						return cb(null, doc);
					}
					if (err) {
						...
						if (
							opts.idStrategy === "uuid_v1" &&
							err.code === couchbase.errors.keyAlreadyExists &&
							idPassedIn === false
						) {
							return self.create(
								connection,
				...
				// Here the ID or Document Key is Generated.
				// You may specify a method to generate ids in the connection configuration in sails.
				...
				connections.getNewId(
					...
					function(id) {
						opts.removePK = true;
						values[collections[collection].primaryKey] = id;
						// Here I'm recursively calling the create method with the newly created ID. By convention,
						// The adapter will not store PKs in the document unless they are included in the
						// values of the document to create, like for instance a custom PK created by the
						// waterline integration tests. So it will be marked to remove. In a More Common scenario,
						// the PKs will never be custom made, and even if they are, the will be created by a custom
						// strategy specified in the configuration of the connection in sails.
						return self.create(connection, collection, values, opts, cb);
					}
				);
			}
		},
		...
		update: function(connection, collection, options, values, cb) {
			...
			if (
				...
				options.where &&
				options.where[collections[collection].primaryKey] &&
				!Array.isArray(options.where[collections[collection].primaryKey])
			) {
				//prepare de id
				options.where[collections[collection].primaryKey] = utils.prepareIds(
					options.where[collections[collection].primaryKey],
					collection
				);
				//make the request
