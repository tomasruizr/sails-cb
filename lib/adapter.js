...
module.exports = (function () {
  ...
  var adapter = {
    ...
    registerConnection: function (connection, colls, cb) {
      ...
      // Register Connection.
      var conn = connections.add(connection, this.defaults);
      var strategy = conn.options.idStrategy || this.defaults.idStrategy;
      // Register Collections.
      async.forEachOf(colls, function (atts, name, next) {
    ...
    create: function (connection, collection, values, options, cb) {
      ...
      var idPassedIn = (!!values[collections[collection].primaryKey] && !opts.removePK);
      
      var id;
      // In case the idStrategy is not increment avoid the recursive call to generate a new ID.
      if (opts.idStrategy !== 'increment' && !values[collections[collection].primaryKey]){
      ...
      if (values[collections[collection].primaryKey] || id){
        ...
        conn.bucket.insert(id, values, opts, function (err) {
          function insertResponse() {
            ...
            doc[collections[collection].primaryKey] = values[collections[collection].primaryKey] || id;
            return cb(null, doc);
          }
          if (err) {
            ...
            if (opts.idStrategy === 'uuid_v1' && err.code === couchbase.errors.keyAlreadyExists && idPassedIn === false) {
               return self.create(connection, collection, values, opts, insertResponse);
            } else {
        ...
        connections.getNewId(conn.bucket, collection, options.idStrategy, function(id) {
          opts.removePK = true;
