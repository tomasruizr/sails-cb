...
module.exports = (function () {
  ...
  var adapter = {
      ...
      connections.DesDocBy_id(bucket, function(err) {
        if (err){
          ...
          bucket.query(vquery, function (error, results) {
            ...
            console.log('Found ' + results.length + ' documents to delete in ' + collection);
    ...
    find: function (connection, collection, options, cb) {
      ...
      if (id && !Array.isArray(id) && _.keys(options.where).length === 1) {
        ...
        conn.bucket.get(id, opts, function (err, res) {
          ...
          debug('VALUE NORMALIZED',value);
          value.id = id;
          // In Case is not a custom pk
          // if (!collections[collection].definition[collections[collection].primaryKey].primaryKey){
          //   debug('Le estoy colocando el id');
          //   value[collections[collection].primaryKey] = id;
          // } else{
          //   debug('no le coloque el id');
          // }
          cb(null, value);
        });
      } else {
    ...
    create: function (connection, collection, values, options, cb) {
      ...
      // console.log(collections[collection].primaryKey);
