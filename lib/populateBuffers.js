...
var strategies = require('waterline-cursor/cursor/strategies');

/**
 * Based on the default populateBuffers implementation. Some minor changes were made to make it work.
 */
  ...
  async.auto({
    ...
    processParent: function(next) {
      ...
      $find(parentCollection, parentCriteria, function(err, results) {
        ...
        parentRecords = Array.isArray(results) ? results: [results];
    ...
    buildChildBuffers: ['processParent', function(results, next) {
      async.each(_.keys(instructions.instructions), function(population, nextPop) {
        ...
        async.mapSeries(parentRecords, function(parent, nextParent) {
          var buffer = {
            attrName: population,
            parentPK: parent[pk],
            pkAttr: pk,
          ...
          // ensure that falsy == undefined && workaround to avoid undefined == {} in tests.
          if (!!buffer.parentFK === false && strategy !== strategies.VIA_JUNCTOR){
            buffer.parentFK = undefined;
            delete parent[buffer.keyName];
          }
    ...
    processChildren: ['buildChildBuffers', function(results, next) {
        ...
        // General case for the other strategies:
          ...
          $find(childIdentity, criteriaToPopulateBuffer, function _afterFetchingBufferRecords(err, childRecordsForThisBuffer) {
            ...
            if (strategy === strategies.VIA_FK){
              buffer.records = Array.isArray(childRecordsForThisBuffer) ? childRecordsForThisBuffer : [childRecordsForThisBuffer];
            }else{
              buffer.records = childRecordsForThisBuffer;
            }
